create external table if not exists wzwd.wzwd_ods_game_Error_d (
    ...
    STORED AS TEXTFILE
    LOCATION '/staging/wzwd/nm/k8s/Error';

ALTER TABLE wzwd.wzwd_ods_game_Error_d add if not exists PARTITION(nm_version='${nm_version}')
    LOCATION '/staging/wzwd/nm/k8s/Error/${nm_version}';
